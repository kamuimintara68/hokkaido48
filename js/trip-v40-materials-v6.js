...
function parseOrganizedTranscript(text) {
    ...
    const result = Object.fromEntries(headers.map(([key]) => [key, ""]));
    const matches = [];
    ...
    if (matches.length < headers.length) {
        throw new Error(
            "必要な5見出しを確認できません。ChatGPTの整理結果が指定形式になっているか確認してください。"
        );
    }
    ...
    matches.forEach(match => {
        if (!firstMatchByKey.has(match.key)) {
            firstMatchByKey.set(match.key, match);
        }
    });

    if (firstMatchByKey.size !== headers.length) {
        throw new Error("同じ見出しの重複または不足があります。");
    }

    const ordered = [...firstMatchByKey.values()].sort((a, b) => a.index - b.index);

    ordered.forEach((match, index) => {
        const next = ordered[index + 1];
        result[match.key] = text
            .slice(match.end, next ? next.index : text.length)
            .trim();
    });
...
async function loadOrganizedTranscript() {
    ...
    try {
        ...
        transcriptPreviewGrid.hidden = true;
        transcriptOrganizerMessage.textContent = error.message;
    }
}
