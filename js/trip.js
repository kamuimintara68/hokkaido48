...
function deleteTrip() {
    ...
    const confirmed = window.confirm("このTripを削除しますか？");

    if (!confirmed) {
        return;
    }

    const trips = TripData.getTrips().filter(trip => trip.id !== id);
    TripData.saveTrips(trips);
...
async function initialize() {
    ...
    console.log(
        "北海道48路線ふらふらlog Trip Version3.3 Ready"
    );
}
