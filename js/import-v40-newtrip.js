(function () {
  ...
  function restoreTripStorage(rawValue) {
    if (rawValue === null) {
      localStorage.removeItem(TRIP_STORAGE_KEY);
    } else {
      localStorage.setItem(TRIP_STORAGE_KEY, rawValue);
    }
  }
  ...
  function saveMaterialInformation() {
    ...
    const originalTripRaw = localStorage.getItem(TRIP_STORAGE_KEY);
    ...
    try {
      ...
      try {
        restoreTripStorage(originalTripRaw);
        resetSavePreparation(
          `${error.message} 元のTripデータへ戻しました。`
        );
      } catch (rollbackError) {
  ...
  reloadTripsButton.addEventListener("click", () => {
    ...
    loadTripCandidates();
    materialSaveMessage.textContent =
      "保存済みTripを再読み込みしました。対象Tripを確認してください。";
  });
  ...
  window.addEventListener("storage", event => {
    if (event.key && isManagedKey(event.key)) {
      if (event.key === TRIP_STORAGE_KEY && latestAnalysis) {
        loadTripCandidates();
        materialSaveMessage.textContent =
          "別のタブで保存されたTripを読み込みました。対象Tripを確認してください。";
        return;
      }
