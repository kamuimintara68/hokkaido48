...
function parseOrganizedTranscript(text) {
    ...
    positions.forEach((item, index) => {
        ...
        if (item.key === "ferment") {
            const originalStart = text.indexOf(
                "----- 原文ここから -----",
                contentStart
            );

            if (originalStart >= 0 && originalStart < contentEnd) {
                contentEnd = originalStart;
            }
        }
...
async function loadOrganizedTranscript() {
    ...
    try {
        ...
        if (
            text.includes("----- 原文ここから -----") ||
            text.includes("----- 原文ここまで -----")
        ) {
            throw new Error(
                "これはChatGPT整理用の元TXTです。ChatGPTで整理した後のTXTを選択してください。"
            );
        }
        ...
        transcriptOrganizerMessage.textContent =
            `整理結果「${file.name}」を読み込みました。` +
            "5項目を確認・修正してから「確認した内容をTrip入力欄へ反映」を押してください。";
